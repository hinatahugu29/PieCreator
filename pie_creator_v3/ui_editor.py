...
TYPE_THEME = {
    ...
    'DIALOG': {'icon': 'WINDOW',           'color': (0.1, 0.8, 0.4, 1.0)}, # 緑
    'STACK':  {'icon': 'LINENUMBERS_ON',   'color': (1.0, 0.6, 0.1, 1.0)}, # オレンジ
    'STICKY': {'icon': 'PINNED',           'color': (0.8, 0.2, 0.8, 1.0)}, # 紫
    'POPUP':  {'icon': 'MENU_PANEL',       'color': (0.5, 0.5, 0.5, 1.0)}, # グレー
}

def draw_sidebar(layout, config, context):
    """左側のメニューリストを階層ツリー形式で表示"""
    menus = config.get("menus", [])
    active_deck_id = config.get("active_deck", "default")
    wm = context.window_manager
    
    col = layout.column(align=True)
    col.label(text="Navigator", icon='OUTLINER')
    ...
    for i, entry in enumerate(ordered_hierarchy):
        ...
        # 選択状態の強調
        is_active = wm.pie_creator_active_menu_id == m["id"]
        if is_active:
            row.active = True
...
def draw_menu_entry(layout, menu, all_menus, config, context, depth=0, drawn_ids=None):
    ...
    wm = context.window_manager
    is_active = wm.pie_creator_active_menu_id == menu_id
    ...
    # --- クイック編集対応の名前表示 ---
    name_op = header.operator("wm.pie_creator_rename_menu", text=menu['name'], icon=theme['icon'], emboss=True, depress=is_active)
    name_op.menu_id = menu_id; name_op.new_name = menu['name']
...
class PIECREATOR_Preferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        # --- Breadcrumbs (親 ➔ 子) の表示 ---
        active_id = wm.pie_creator_active_menu_id
        if active_id:
            ordered_hierarchy = get_menu_hierarchy(menus, active_deck_id)
...
class PIECREATOR_OT_SelectMenu(bpy.types.Operator):
    ...
    menu_id: bpy.props.StringProperty()
    def execute(self, context):
        context.window_manager.pie_creator_active_menu_id = self.menu_id
        return {'FINISHED'}
