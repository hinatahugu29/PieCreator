bl_info = {
    ...
    "version": (0, 8, 0),
    "blender": (5, 0, 0),
    "location": "Preferences > Addons > PieCreator V8",
    "description": "Hybrid Nesting Menu Editor (V8) - Bulk Import & Menu Scraping Edition",
    "category": "Interface",
}
...
def register():
    # 1. Register PropertyGroup classes FIRST (must be registered before used in CollectionProperty)
    from .operators import PIECREATOR_ScrapedItem
    if hasattr(bpy.types, "PIECREATOR_ScrapedItem"):
        old_cls = getattr(bpy.types, "PIECREATOR_ScrapedItem")
        try:
            bpy.utils.unregister_class(old_cls)
        except:
            pass
    try:
        bpy.utils.register_class(PIECREATOR_ScrapedItem)
    except Exception as e:
        print(f"PieCreator: Failed to register PIECREATOR_ScrapedItem: {e}")
    ...
    # Scraping Properties
    bpy.types.WindowManager.pie_creator_scraped_items = bpy.props.CollectionProperty(type=operators.PIECREATOR_ScrapedItem)
    bpy.types.WindowManager.pie_creator_is_scraping = bpy.props.BoolProperty(default=False)
    bpy.types.WindowManager.pie_creator_scrape_menu_id = bpy.props.StringProperty(name="Menu ID to Scrape", default="VIEW3D_MT_mesh_add")
    bpy.types.WindowManager.pie_creator_blender_menus = bpy.props.CollectionProperty(type=bpy.types.PropertyGroup)

    # 2. Register classes and run init logic
    operators.register()
    ui_editor.register()
    
    # Context Menu Add List Class
    if hasattr(bpy.types, "PIECREATOR_MT_ContextMenuAddList"):
        old_cls = getattr(bpy.types, "PIECREATOR_MT_ContextMenuAddList")
        try:
            bpy.utils.unregister_class(old_cls)
        except:
            pass
