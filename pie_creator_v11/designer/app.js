...
function renderMenuSettings() {
    ...
    const delBtn = document.getElementById('btn-delete-menu');
    if (delBtn) {
        const isRoot = currentMenuId === config.menus[0].id;
        delBtn.disabled = isRoot;
        delBtn.style.opacity = isRoot ? '0.3' : '1';
        delBtn.style.cursor = isRoot ? 'not-allowed' : 'pointer';
        delBtn.title = isRoot ? "The Root menu cannot be deleted." : "Delete this menu";
    }
}
...
function generateUniqueId(baseId) {
    let newId = baseId;
    let counter = 1;
    const existingIds = config.menus.map(m => m.id);
...
async function pasteItemsIntoMenu() {
    try {
        const text = await navigator.clipboard.readText();
        if (!text) return;
        const data = JSON.parse(text);
        ...
        if (data.type === "PIE_CREATOR_MENU") {
            sourceMenus = [data.payload];
            itemsToAppend = data.payload.items || [];
        } else if (data.type === "PIE_CREATOR_PROJECT") {
            sourceMenus = data.payload.menus || [];
            if (sourceMenus.length === 0) return;
            itemsToAppend = sourceMenus[0].items || [];
        } else {
            alert("Clipboard does not contain a PieCreator menu/project.");
            return;
        }

        // 1. Prepare unique IDs for all menus being brought in (except the source parent which we are dissolving)
        const idMap = {};
        ...
        // 2. Dissolve items and fix their internal links
        const processedItems = JSON.parse(JSON.stringify(itemsToAppend));
        processedItems.forEach(it => {
            if ((it.type === 'MENU' || it.type === 'SUBMENU') && idMap[it.menu_id]) {
                it.menu_id = idMap[it.menu_id];
            }
            // Generate unique item ID to avoid selection/key collision in Designer
            it.id = 'it_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now();
        });

        // 3. Add necessary sub-menus to the project (those linked by the dissolved items)
        // For simplicity, we add all sourceMenus to the project, 
        // effectively importing the entire "branch" while dissolving the "top" node.
        sourceMenus.forEach(m => {
            config.menus.push(m);
        });

        // 4. Append items to current menu
        const currentMenu = config.menus.find(m => m.id === currentMenuId);
        if (currentMenu) {
            if (!currentMenu.items) currentMenu.items = [];
            currentMenu.items.push(...processedItems);
            
            logToUI(`Appended ${processedItems.length} items directly into '${currentMenu.name}'.`);
            renderAll();
        } else {
            alert("No menu is currently active.");
        }

    } catch (err) {
        alert("Failed to expand-paste: " + err);
        console.error(err);
    }
}
